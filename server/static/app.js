...
const authMenuSettings = document.getElementById("auth-menu-settings");
...
document.addEventListener("click", (event) => {
  if (!authUserDropdown?.classList.contains("is-open")) {
    return;
  }
  if (authUserMenu?.contains(event.target)) {
    return;
  }
  closeAuthMenu();
});
...
const closeAuthMenu = () => {
  ...
  authUserDropdown.classList.remove("is-open");
  authUserDropdown.setAttribute("aria-hidden", "true");
};
...
const toggleAuthMenu = () => {
  ...
  const willOpen = !authUserDropdown.classList.contains("is-open");
  authUserDropdown.classList.toggle("is-open", willOpen);
  authUserDropdown.setAttribute("aria-hidden", willOpen ? "false" : "true");
};

const applyAuthUi = () => {
  if (authLoginButton) {
    const userLabel = state.authUser?.display_name || state.authUser?.username || "ログイン";
    if (authUserDisplay) {
      authUserDisplay.textContent = userLabel;
    }
    if (authUserId) {
      authUserId.textContent = state.authUser ? `@${state.authUser.username}` : "";
      authUserId.style.display = state.authUser ? "" : "none";
    }
    if (authUserIcon) {
...
const bindAuthAdminEvents = () => {
  ...
  authMenuSettings?.addEventListener("click", () => {
    closeAuthMenu();
    activateTab("settings");
  });
  authMenuLogout?.addEventListener("click", async () => {
    closeAuthMenu();
